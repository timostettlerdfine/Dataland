...
function fillRequiredEutaxonomyNonFinancialsFields(): void {
  cy.get(
    ...
    cy.get('#dataPointIsAvailableSwitch')
      .then(($els) => cy.task('log', `[DataUpload] #dataPointIsAvailableSwitch count: ${$els.length}`))
      .should('have.length', 1)
      .click();
  });
  cy.get('[data-test="fiscalYearEnd"] button')
    .then(($btns) => cy.task('log', `[DataUpload] fiscalYearEnd p-datepicker-dropdown buttons count: ${$btns.length}`))
    .should('have.length', 1)
    .should('have.class', 'p-datepicker-dropdown')
    .click();
  cy.get('.p-datepicker').then(($dp) =>
    cy.task('log', `[DataUpload] open .p-datepicker overlays after fiscalYearEnd open: ${$dp.length}`)
  );
  cy.get('.p-datepicker-header')
    .then(($headers) => cy.task('log', `[DataUpload] .p-datepicker-header count: ${$headers.length}`))
    .find('button[aria-label="Next Month"]')
    .then(($btns) => cy.task('log', `[DataUpload] Next Month buttons count: ${$btns.length}`))
    .should('have.length', 1)
    .first()
    .click();
  cy.get('.p-datepicker-day-view')
    .then(($views) => cy.task('log', `[DataUpload] .p-datepicker-day-view count: ${$views.length}`))
    .should('have.length', 1)
    .find('span:contains("11")')
    .then(($spans) => cy.task('log', `[DataUpload] span:contains("11") count: ${$spans.length}`))
    .should('have.length', 1)
    .click();
...
describeIf(
  ...
  function () {
    ...
    // DEBUG: repeat the test 6 times to reproduce the flaky click error reliably; remove the loop after debugging
    for (let debugRun = 1; debugRun <= 6; debugRun++) {
      it(
        `[Run ${debugRun}/6] Check if the file upload info remove button works as expected, make sure the file content hashes ` +
